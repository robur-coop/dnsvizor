let ip =
  ...
  Asn.S.(
    map f g
      (choice2 (explicit 0 int)
         (explicit 1
            (sequence4 (required int) (required int) (required int)
               (required int)))))

let sni =
  let f (sni, host, port) =
    (Domain_name.(host_exn (of_string_exn sni)), host, port)
  and g (sni, host, port) = (Domain_name.to_string sni, host, port) in
  Asn.S.(
    map f g
      (sequence3
...
let data =
  let f = function `C1 s -> s | `C2 () -> assert false and g s = `C1 s in
  Asn.S.(map f g (choice2 (sequence_of sni) (explicit 1 null)))

let decode_strict codec cs =
  match Asn.decode codec cs with
  | Ok (a, rest) ->
      if String.length rest = 0 then Ok a else Error (`Msg "trailing bytes")
...
let add_sni snis (sni, host, port) =
  ...
  | Some (ohost, oport) ->
      Logs.warn (fun m ->
          m "overwriting %a -> %a:%d with %a:%d" Domain_name.pp sni Ipaddr.pp
            ohost oport Ipaddr.pp host port));
  Logs.info (fun m ->
      m "%a is now redirected to %a:%d" Domain_name.pp sni Ipaddr.pp host port);
...
let cmd =
  ...
  Asn.S.(
    map f g
      (choice5 (explicit 0 sni) (explicit 1 utf8_string) (explicit 2 null)
