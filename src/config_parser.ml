...
type config_item = [ `Dhcp_range of dhcp_range | `Dhcp_host of dhcp_host | `Domain of domain | `Dhcp_option of dhcp_option | `No_hosts | `Dnssec ]

type config = [ config_item | `Ignored ] list

let pp_config_item mode ppf item =
  let pfx = match mode with
    | `File -> Fmt.nop
    | `Arg -> Fmt.any "--"
  in
  pfx ppf ();
  ...
  | `Dhcp_host dhcp_host ->
    Fmt.pf ppf "dhcp-host=%a" pp_dhcp_host dhcp_host
  | `Domain domain ->
    Fmt.pf ppf "domain=%a" pp_domain domain
  | `Dhcp_option dhcp_option ->
    Fmt.pf ppf "dhcp-option=%a" pp_dhcp_option dhcp_option
  | `No_hosts ->
    Fmt.string ppf "no-hosts"
  | `Dnssec ->
    Fmt.string ppf "dnssec"

let pp_config mode ppf config =
  let sep = match mode with
    | `File -> Fmt.any "\n"
    | `Arg -> Fmt.any " "
  in
  Fmt.(list ~sep) (pp_config_item mode) ppf
    (List.filter_map (function `Ignored -> None | #config_item as item-> Some item) config)
