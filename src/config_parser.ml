...
let lease_time =
  (int >>= fun n ->
   ...
   if r > 0 && r < infinite then return r
     ...
     fail (Fmt.str "Value %u (from %S%s) does not fit into 32 bits" r (string_of_int n) c))
...
let address_range =
  ...
  ((string "," *> ipv4_dotted >>= fun stop -> return (`Ip_range (start, stop)))
   <|> (string "/" *> int >>= fun prefix -> return (`Ip (Ipaddr.V4.Prefix.make prefix start))))
...
let domain end_of_directive =
  (take_while1 (function ',' -> false | _ -> true) >>= fun name ->
   ...
   | Error (`Msg e) ->
     fail (Fmt.str "Invalid domain: %s: %S" e name)) >>= fun domain ->
  option None
    (string "," *>
     ((address_range >>= fun ar -> return (Some ar)) <|>
      (take_while (function '\n' -> false | _ -> true) >>= fun intf -> return (Some (`Interface intf))))) >>= fun snd ->
  end_of_directive >>| fun () ->
  (domain, snd)
...
let domain_c =
  conv_cmdliner ~docv:domain_docv
    (domain arg_end_of_directive)
    pp_domain
